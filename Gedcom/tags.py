...
# Templates:
propertyT = Template('@property $type *$name;$description')
dynamicT = Template('@dynamic $name;')
forwardT = Template('@class $name;')

declarationT = Template('+($returnType *)$name$extra')
bodyT = Template("""{
...
definitionT = Template("""@interface $name : $superClass
...
def property(key, type, is_plural, is_required, is_super_variant=False):
	...
	description = '%s%s' % ('array of %s' % classify(key, type) if is_plural and not is_super_variant else '', ' NB: required property' if is_required else '')

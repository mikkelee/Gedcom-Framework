...
- (void)setPedigree:(id)obj
{
	if (!_isBuildingFromGedcom) {
		...
		@synchronized (uM) {
			...
			[(GCChildInFamilyRelationship *)[uM prepareWithInvocationTarget:self] setPedigree:_pedigree];
	...
	if (_pedigree) {
		[(id)_pedigree setValue:nil forKey:@"describedObject"];
	}
...
- (void)insertObject:(id)obj inNoteReferencesAtIndex:(NSUInteger)idx
{
	...
	if (!_isBuildingFromGedcom) {
		...
		@synchronized (uM) {
			[uM beginUndoGrouping];
			[(GCChildInFamilyRelationship *)[uM prepareWithInvocationTarget:self] removeObjectFromNoteReferencesAtIndex:idx];
			[uM setActionName:[NSString stringWithFormat:GCLocalizedString(@"Undo %@", @"Misc"), self.localizedType]];
			[uM endUndoGrouping];
		}
	}
...
- (void)removeObjectFromNoteReferencesAtIndex:(NSUInteger)idx
{
	if (!_isBuildingFromGedcom) {
		...
		@synchronized (uM) {
			[uM beginUndoGrouping];
			[(GCChildInFamilyRelationship *)[uM prepareWithInvocationTarget:self] insertObject:_noteReferences[idx] inNoteReferencesAtIndex:idx];
			[uM setActionName:[NSString stringWithFormat:GCLocalizedString(@"Undo %@", @"Misc"), self.localizedType]];
			[uM endUndoGrouping];
		}
	}
...
- (void)insertObject:(id)obj inNoteEmbeddedsAtIndex:(NSUInteger)idx
{
	...
	if (!_isBuildingFromGedcom) {
		...
		@synchronized (uM) {
			[uM beginUndoGrouping];
			[(GCChildInFamilyRelationship *)[uM prepareWithInvocationTarget:self] removeObjectFromNoteEmbeddedsAtIndex:idx];
			[uM setActionName:[NSString stringWithFormat:GCLocalizedString(@"Undo %@", @"Misc"), self.localizedType]];
			[uM endUndoGrouping];
		}
	}
...
- (void)removeObjectFromNoteEmbeddedsAtIndex:(NSUInteger)idx
{
	if (!_isBuildingFromGedcom) {
		...
		@synchronized (uM) {
			[uM beginUndoGrouping];
			[(GCChildInFamilyRelationship *)[uM prepareWithInvocationTarget:self] insertObject:_noteEmbeddeds[idx] inNoteEmbeddedsAtIndex:idx];
			[uM setActionName:[NSString stringWithFormat:GCLocalizedString(@"Undo %@", @"Misc"), self.localizedType]];
			[uM endUndoGrouping];
		}
	}
